...
KvFlatBtreeAsync(int k_val, string name, int cache, double cache_r,
  ...
  /**
   * This guarantees consistency if no clients die, but it does not guarantee
   * forward progress. It follows the same general logic as a rebalance, but
   ...
   * copies all of the entries in each object it modifies.
   *
